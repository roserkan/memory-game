...
export default function GameType() {
        ...
        add(values)
            .then(res => {
                if (res.data.status === 200) {
                    toastr('success', 'Başarılı', 'Oyun Tipi eklendi')
                    allData()
                    addForm.resetFields()
    ...
    const updating = values => {
        update(currentId, values)
            .then(res => {
                if (res.data.status === 200) {
                    toastr('success', 'Başarılı', 'Oyun tipi güncellendi')
                    allData()
                } else if (res.data.message === 'UNIQUE ERROR') {
    ...
    const deleting = (id) => {
        remove(id)
            .then(res => {
                if (res.data.status === 200) {
                    toastr('success', 'Başarılı', 'Oyun tipi silindi')
                    allData()
                } else {
                    toastr('success', 'Hata', 'Beklenmeyen hata!')
                }
    ...
    return (
        <div>
            ...
            <Modal title="Oyun Tipi Düzenle" visible={updateModalVisible} onCancel={closeModel} footer={footer}>
